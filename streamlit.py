"""
Viticultura Dashboard — Streamlit
=================================


Aplicação para análise interativa do setor de vitivinicultura com autenticação via API local,
consumo de endpoints históricos e página de previsões (backtest ETS + previsão 5 anos) a partir
da API /sintese ou de um CSV enviado pelo usuário.


Principais bibliotecas: Streamlit, Plotly, Pandas, NumPy, Requests, Statsmodels (ETS), scikit‑learn,
...
Configurar a API em `API_BASE`. Endpoints esperados:
- Auth: /users/login (POST form), /users/signup (POST JSON)
- Dados: /importacoes/, /exportacoes/, /producoes/, /processamentos/, /comercializacoes/
- Síntese anual: /sintese/


Este arquivo contém **docstrings** em todas as funções e comentários de seção para facilitar
...
def inject_css():
    """
Injeta estilos CSS no app para padronizar a interface.


O CSS define componentes como:
- Cards de métrica (bordas, sombras, tamanhos de fonte);
- Seção "hero" com imagem de fundo e gradiente;
- Caixas de callout/spotlight para storytelling;
- Estilo do menu lateral.


Observação: usa `unsafe_allow_html=True` para permitir HTML/CSS no Streamlit.
    """
    st.markdown("""
    <style>
      :root { --brand:#e53935; }
...
def metric_card(title, value, sub=None):
    """
        Padroniza um card para metricas
    """
    st.markdown(f"""
...
def human_compact(v, sufixo=""):
    """
        Formata os numeros para numeros compactos
    """
    try:
        n=float(v)
...
# API LOCAL
# -----------------------------------------------------------------------------
API_BASE = "http://localhost:10000/api/v1"
...
SPOTLIGHTS = {
    ...
    "Processamento": ("Safra 2024 marcada por clima extremo exige eficiência de planta e priorização de rótulos core "
...
def render_intro_navigation():
    """
        Padroniza a pagina inicial da aplicação, explicando sobre o que vai se encontrar na aplicação
    """
    st.markdown("""
<div class="story">
...
def cadastrar_usuario(name, surname, email, password):
    """
        Registra um novo usuario, em caso de sucesso, realiza o login automático
    """
    try:
...
def _auth_headers():
    """
        Monta headers de autenticação para chamadas autenticadas.
    """
...
def carregar_dados_com_silencio(endpoint):
    """
        Wrapper tolerante para carregar dados
        Captura exceções, exibe erro amigável e retorna `DataFrame()` vazio em caso de falha.
    """
    try:
...
def detectar_coluna_produto(df: pd.DataFrame):
    """
    Tenta identificar a coluna de produto no DataFrame.

    Procura, na ordem, por: `produto`, `produto_nome`, `produto_tipo`.
    Retorna o nome da coluna encontrada ou `None` se nenhuma existir.
    """
...
def aplicar_filtros(df: pd.DataFrame, produto_col: str, busca, anos, produtos):
    """
        Aplica filtros de produto/país, intervalo de anos e termos de busca
    """
    df = df.copy()
...
def _key_from(parts):
    """
        Gera uma chave estável para componentes gráficos do Streamlit.
        Concatena as partes fornecidas com `_`, em minúsculas e sem espaços, evitando colisões
        ao alternar abas/páginas.
    """
...
def set_title_and_subtitle(fig, title, subtitle=None):
    """Aplica título (e subtítulo opcional) ao layout do gráfico.
    O subtítulo é inserido como `<sup>` na propriedade de título para melhor legibilidade.
    """
    if subtitle:
...
def annotate_line_peaks(fig, df_xy, xcol, ycol):
    """
    Apresenta altos e baixos do gráfico
"""
...
def fmt_mi(v, sufixo=""):
    """Formatação alternativa compacta (inteira) para MI/K/valor base."""
    try:
        n = float(v)
...
# -----------------------------------------------------------------------------
# Storytelling + gráficos
# -----------------------------------------------------------------------------
def _years_info(df):
    """
    Extrai informações básicas de anos do DataFrame.
    """
...
def _top_group(df, group_col, metric, n=5):
    """
    Retorna a série com Top N itens por `metric`, agrupados por `group_col`.
    Caso não seja possível (colunas ausentes ou `df` vazio), retorna `Series(dtype=float)`.
    """
...
def render_left_menu():
    """
        Renderiza menu lateral com `streamlit-option-menu` e botão de logout.
        Atualiza `st.session_state.page` conforme seleção; no logout, limpa token e volta para "Início".
    """
    with st.container():
...
def filter_panel(produto_col, df):
    """
        Constrói o painel de filtros (busca, intervalo de anos, produtos).
    """
...
def fit_ets(y_train: pd.Series):
    """
        Ajusta um modelo ETS (Holt‑Winters) com tendência aditiva amortecida.
        Pré‑processa a série para não permitir valores negativos e usa inicialização automática.
    """
...
def rolling_backtest(df_year_val: pd.DataFrame, start_year: int, end_year: int) -> pd.DataFrame:
    """
    Executa backtest 1‑passo‑à‑frente com janela crescente.
    Para cada ano `ano_t` em `[start_year, end_year]`:
    1) treina ETS com dados de anos `< ano_t` (requer pelo menos 4 pontos);
    2) prevê 1 passo para `ano_t`;
    3) armazena `previsto_bruto` e `real`.
    """
    out_rows = []
...
# 2) Com login
    ...
    with col_content:
        ...
        if page == "Previsões":
            ...
            tab_bt, tab_fc = st.tabs(["Backtest", "Previsão 5 anos"])

            with tab_bt:
                modo = st.radio("Fonte de dados para o backtest", ["API (/sintese)"], horizontal=True)
                ...
                if modo == "API (/sintese)":
                    ...
                    def carregar_sintese(api_url: str, ano_min=None, ano_max=None) -> pd.DataFrame:
                        """
                            Lê a síntese anual da API para compor série por ano.
                        """
                        params = {}
                    ...
                    if st.button("Rodar Backtest (API)", key="run_backtest_api"):
                        ...
                        if calib is not None:
                            st.caption(f"Calibração linear: previsto = {calib[0]:.2f} + {calib[1]:.3f} × previsto")

                        st.subheader(f"Tabela — Backtest ({col_val})")
                        st.dataframe(out, use_container_width=True)

                        fig = px.line(out, x="ano", y="real", markers=True,
                                    title=f"Backtest {int(ini)}–{int(fim)} — Real vs Previsto{col_val}")
                ...
                with tab_fc:
                    st.markdown("---")
                    st.subheader("Previsão (5 anos) — escolha o modelo")

                    X = serie["ano"].values.reshape(-1, 1)       
                    ...
                    passos = 5
                    anos_fc = np.arange(serie["ano"].max() + 1, serie["ano"].max() + 1 + passos).reshape(-1, 1)
                    fc = modelo_lr.predict(anos_fc)
                    ...
                    fig_fc = px.line(hist, x="ano", y="real", markers=True,
                                    title=f"Série histórica + Regressão Linear (5 anos) • {col_val}")
                    fig_fc.add_scatter(x=prev_df["ano"], y=prev_df["previsao"],
                                    mode="lines+markers", name="Previsão 5 anos")
                    chart(fig_fc, key_parts=["previsoes", "api_forecast5", col_val])

                    st.dataframe(prev_df)
                    st.write("Por que usamos Regressão Linear:** após testar modelos tradicionais (Holt‑Winters) e algoritmos de máquina de árvore (Random Forest), escolhemos a regressão linear simples para a previsão de cinco anos.") 
                    st.write("A regressão linear ajusta uma tendência clara ao longo dos anos e consegue extrapolar essa tendência para anos futuros. Já modelos de árvore, como o Random Forest, são “locais” e fazem previsões com base apenas nas amostras vistas no treino; por isso tendem a não extrapolar bem para além do horizonte observado:contentReference[oaicite:3]{index=3}, produzindo séries quase constantes:contentReference[oaicite:4]{index=4}. Para séries curtas e métricas anuais, a regressão linear é uma escolha mais robusta e proporciona previsões que refletem o ritmo de crescimento observado historicamente.")
        ...
        with aba_pareto:
            ...
            if group_col and col_metric in df_filtrado.columns:
                ...
                if total > 0:
                    ...
                    figp.update_xaxes(showticklabels=False, title=None)  
